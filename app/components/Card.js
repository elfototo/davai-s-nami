...
const Card = ({ type, category, title, from_date, address, price, id, image }) => {
  ...
  return (
    <div className='group'>
      <div className=" w-full h-auto max-w-sm overflow-hidden bg-white rounded-lg border border-[#D9D9D9] group-hover:border-pink-400">
        {/* Header card */}
        ...
        {/* Card content */}
        <div className='min-h-[130px] px-2 pt-2'>
          <div className="mb-3">
            <p
              title={title}
          ...
          <div className='flex items-center justify-between flex-wrap'>
            ...
            <div className='flex flex-col items-start'>
              <div className='flex items-center'>
                ...
                <p className="text-[#777] font-roboto">{from_date ? dayjs(from_date).format('DD MMMM') : 'Скоро будет дата'}</p>
              </div>
        ...
        <Link href={`/events/${id}`}  >
          <button className='font-roboto my-2 mx-3 w-[90%] py-2 text-[1rem] font-medium bg-pink-500 text-[#fff] rounded-lg transform transition-transform duration-300 hover:bg-pink-400'>Смотреть</button></Link>
      </div>
